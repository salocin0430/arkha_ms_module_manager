    ...
    for arka in arkas_resultado:
        ...
        if numero_arka == 1:
            ...
            if arka["direccion_anterior"] == "ARRIBA":
                posicion_base_actual = np.array([0, 0, 4*constantes["ancho_modelo"]]) + posicion_base_anterior
            elif arka["direccion_anterior"] == "IZQ":
                posicion_base_actual = np.array([4*constantes["ancho_modelo"], 0, 0]) + posicion_base_anterior
...
def generar_json_solo_001_011_004(arkas_resultado, passengers=30, duration=500, isScientific=True):
    ...
    modulos_adicionales, pos_base_anterior, pos_base_actual = añadir_modulos_por_arka(arkas_resultado)
